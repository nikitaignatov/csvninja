...
// csv library does cannot handle leading whitespace when headers are quoted like in this example https://people.sc.fsu.edu/~jburkardt/data/csv/trees.csv
const trim = function (input: string): string {
    try {
        let [head, ...tail] = input.split('\n');
        head = head.replace(/",\s*"/gi, '","');
